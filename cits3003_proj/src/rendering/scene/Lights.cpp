...
template<typename Light>
std::vector<Light> LightScene::get_nearest_lights_spot(const std::unordered_set<std::shared_ptr<Light>>& lights, glm::vec3 target, size_t max_count, size_t min_count) {
    if (lights.size() <= max_count) {
        // No need to store if we are just going to return them all anyway.
