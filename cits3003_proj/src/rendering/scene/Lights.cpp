...
std::vector<DirectionLight> LightScene2::get_nearest_direction_lights(glm::vec3 target, size_t max_count, size_t min_count) const {
    return get_nearest_lights(direction_lights, target, max_count, min_count);
}


template<typename Light>
std::vector<Light> LightScene2::get_nearest_lights(const std::unordered_set<std::shared_ptr<Light>>& lights, glm::vec3 target, size_t max_count, size_t min_count) {
    if (lights.size() <= max_count) {
        // No need to store if we are just going to return them all anyway.

        std::vector<Light> result{};
        result.reserve(std::max(lights.size(), min_count));
        for (const auto& direction_light: lights) {
            result.push_back(*direction_light);
        }
        while (result.size() < min_count) {
            result.push_back(Light::off());
        }
    ...
    sorted_vector.reserve(lights.size());
    for (const auto& direction_light: lights) {
        glm::vec3 diff = direction_light->direction - target;
        // dot(a, a) == |a|^2, but doesn't require a sqrt()
        // and since (|a| < |b|) <-> (|a|^2 < |b|^2) so sorting works with squares
        float distance_squared = glm::dot(diff, diff);
        sorted_vector.emplace_back(distance_squared, *direction_light);
    }
