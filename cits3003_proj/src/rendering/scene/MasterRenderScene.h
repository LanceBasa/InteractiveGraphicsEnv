...
public:
    ...
    void insert_light(std::shared_ptr<DirectionalLight> directional_light); // New method for inserting directional lights

    bool remove_light(const std::shared_ptr<PointLight>& point_light);
    bool remove_light(const std::shared_ptr<DirectionalLight>& directional_light); // New method for removing directional lights
