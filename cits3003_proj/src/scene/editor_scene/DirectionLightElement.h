#ifndef DIRECTION_LIGHT_ELEMENT_H
#define DIRECTION_LIGHT_ELEMENT_H

#include "SceneElement.h"
#include "scene/SceneContext.h"

namespace EditorScene {
    class DirectionLightElement : public SceneElement {
    public:
        ...
        DirectionLightElement(const ElementRef& parent, std::string name, glm::vec3 position, std::shared_ptr<DirectionLight> light, std::shared_ptr<EmissiveEntityRenderer::Entity> light_sphere) :
            SceneElement(parent, std::move(name)), position(position), light(std::move(light)), light_sphere(std::move(light_sphere)) {}

        static std::unique_ptr<DirectionLightElement> new_default(const SceneContext& scene_context, ElementRef parent);
        static std::unique_ptr<DirectionLightElement> from_json(const SceneContext& scene_context, ElementRef parent, const json& j);
...
#endif //DIRECTION_LIGHT_ELEMENT_H
